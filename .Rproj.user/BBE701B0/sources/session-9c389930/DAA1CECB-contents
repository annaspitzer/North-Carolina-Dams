...
short_summary_county <- finaldata_county %>%
  ...
  mutate(SOVI_category = case_when(
    SOVI > 0.75 ~ "County SVI > 0.75",
    SOVI < 0.25 ~ "County SVI < 0.25",
    TRUE ~ "All counties"
  ))

long_summary_county <- finaldata_county %>%
  group_by(CountyID, County) %>%
  summarise(
...
summary_basin <- finaldata_county %>%
  group_by(Basin_Name) %>%
  summarise(
    `High Hazard Dams (HHD)` = sum(Hazard_Potential_Classification == "High", na.rm = TRUE),
    ...
    # `HHD / Person` = `High Hazard Dams (HHD)` / Population,
    `Mean Distance to City` = mean(Distance_to_Nearest_City_Miles, na.rm = TRUE),
    `Mean Storage` = mean(`Normal_Storage_Acre_Ft`, na.rm = TRUE),
